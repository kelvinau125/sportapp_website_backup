  ...
  <div class="flex gap-1 flex-wrap">
    <div class="" v-for="match in matchDetails" :key="match.matchDetails">
      <div
        ...
        style="width: 204px; height: 136px; border-radius: 8px"
      >
        <!-- 热门赛程 Header -->
        <div
          class="py-1.5 flex justify-start items-center rounded-t-lg"
          style="background-color: #d6f1dd; height: 30px"
        >
          <div class="headerContainer pl-2.5 overflow-clip">
            <div
              class="headerBorder text-xs font-medium flex justify-center text-start pl-1.5 pt-0.5"
            >
              <span
        ...
        <!-- 热门赛程 Contents -->
        <div
          class="flex justify-between rounded-b-lg bg-white w-full"
          style="height: 106px"
        >
          <div
            class="flex flex-col justify-center items-center pb-3 h-full"
            style="width: 100px"
          >
            <div class="text-base font-semibold h-1/3 pt-2">
              {{ match.homeTeamScore }}
            </div>
            ...
            <div
              class="w-[60px] h-[100%] flex justify-center text-center text-xs font-normal text-grayText"
            >
              <span class="whitespace-nowrap overflow-hidden text-elleipsis">{{
                match.homeTeamName
              }}</span>
            </div>
          </div>
          <div class="flex relative flex-col justify-start items-center py-2">
            <div
              v-show="isCN"
              class="w-[80px] h-1/2 absolute font-medium text-sm pt-3 flex items-center justify-center"
              :class="{
                statusStartBorder: match.status === '开',
                statusEndBorder: match.status === match.status,
              }"
            >
              <span class="whitspace-normal overflow-hidden text-ellipsis">{{
                match.status
              }}</span>
            </div>
            ...
            <div
              v-show="!isCN"
              class="w-[80px] h-1/2 absolute font-medium text-xs pt-3 flex items-center justify-center"
              :class="{
                statusStartBorder: match.status === ('Started' || 'Start'),
                statusEndBorder: match.status === match.status && match.status !== '',
              }"
            >
              <span class="whitspace-normal overflow-hidden text-ellipsis">{{
                match.status
          ...
          <div
            class="flex flex-col justify-center items-center pb-3"
            style="width: 100px"
          >
            <div class="text-base font-semibold h-1/3 pt-2">
              {{ match.awayTeamScore }}
            ...
            <div
              class="w-[60px] h-[100%] flex justify-center text-center text-xs font-normal text-grayText"
            >
              <span class="multiline-ellipsis">{{ match.awayTeamName }}</span>
            </div>
          </div>
        </div>
      </div>
    ...
    >
      ...
      <div class="flex justify-center" style="width: 31px; height: 36px">
        <span class="pt-4 text-center text-xs font-medium text-white">{{
          $t("All fixtures")
        }}</span>
      </div>
    </div>
...
import {
  getBasketBallMatchTodaybyCompName,
  getBasketballMatchByDate,
} from "@/service/apiBasketBallMatchProvider.js";
