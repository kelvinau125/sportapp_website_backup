<template>
  <footer class="bg-black text-white pt-3 " style="height: 103px;">
    <div class="max-w-7xl m-auto flex  flex-col  items-center" >
      <div  class="footer-links flex items-center mt-1 text-grayText">
        <p class="text-xs font-normal px-1">用户服务</p>
        <span>|</span>
        <p class="text-xs font-normal px-1">协议隐私政策</p>
        <span>|</span>
        <p class="text-xs font-normal px-1">免责声明</p>
        <span>|</span>
        <p class="text-xs font-normal px-1">权限详情</p>
        <span>|</span>
        <p class="text-xs font-normal px-1">联系我们</p>
        <span>|</span>
      </div>
      <div  class="text-xs font-normal text-grayText">
        本站所有直播均来自互联网，本站不从事任何经营业务，仅为体育爱好者提供免费赛事直播，如果侵犯了您的权益，请联系我们
      </div>
      <div  class="text-xs font-normal text-grayText mt-3">
        Copyright © 2021 All Rights Reserved

      </div>
      <!-- <div class="footer-links flex items-center text-grayText">
        <p class="text-xs font-normal px-1">用户服务</p>
        <span>|</span>
        <p class="text-xs font-normal px-1">协议隐私政策</p>
        <span>|</span>
        <p class="text-xs font-normal px-1">免责声明</p>
        <span>|</span>
        <p class="text-xs font-normal px-1">权限详情</p>
        <span>|</span>
        <p class="text-xs font-normal px-1">联系我们</p>
        <span>|</span>
      </div>
      <div class="text-xs font-normal mb-2.5 text-grayText">
        本站所有直播均来自互联网，本站不从事任何经营业务，仅为休育爱好者提供免费赛事直播，如果侵犯了您的权益，请联系我们.
      </div>
      <div class="text-xs font-normal text-grayText">
        Copyright © 2021 All Rights Reserved
      </div> -->
    </div>
  </footer>
</template>


<style>
.footer-links {
  margin-bottom: 5px;
}
</style>
