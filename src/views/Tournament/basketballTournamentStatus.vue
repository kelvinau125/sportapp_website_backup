      ...
      <div class="flex md:flex-col justify-center py-4">
        ...
        <div
          style="
            ...
            height: 32px;
          "
          class="borderColor mb-3 flex justify-center items-center md:mr-0 -mr-5"
          v-show="isCN"
        >
          <span class="font-medium md:text-sm text-xs"
            >{{ $t("Pause") }} {{ teamATotalPause }}</span
          >
        </div>
        <!-- ZH Show -->
        <div
          style="
            ...
            height: 32px;
          "
          class="borderColor flex justify-center items-center md:ml-0 ml-6"
          v-show="isCN"
        >
          <span class="font-medium md:text-sm text-xs"
            >{{ $t("Foul") }} {{ homeNumOfFouls }}</span
          >
        </div>
      </div>
      <div class="flex justify-center">
        <div class="flex flex-col justify-around">
          <div class="flex justify-center items-center">
            <p class="w-[30px] font-medium md:text-sm text-10px md:pr-2 pr-1">
              {{ homeThreeGoal }}
            </p>
          ...
          <div class="flex items-center" v-show="!isCN">
            ...
            <div class="team relative w-[50px] p-[20px] bottom-1">
              ...
              <div
                class="stat-bar-left left-bar-color border_leftTB"
                :style="{ width: teamATotalTurnOver + '%' }"
              ></div>
            </div>
          </div>
          <div class="flex items-center" v-show="!isCN">
          ...
          <div class="flex items-center">
            ...
            <div class="team relative w-[50px] p-[20px] bottom-1">
              ...
              <div
                class="stat-bar-left left-bar-color border_leftTB"
                :style="{ width: teamAFreeThrowPercentage + '%' }"
              ></div>
            </div>
          </div>
        </div>
        <div class="flex flex-col justify-around items-center md:w-[180px] w-[100px]">
          ...
          <!-- ZH Need Show -->
          <div class="font-medium md:text-sm text-xs" v-show="isCN">
            {{ $t("2 Pointer") }}
          </div>
          ...
          <div class="flex justify-start items-center w-full" v-show="isCN">
            <div class="font-medium md:text-sm text-xs w-1/3 md:pl-3 pl-1">
              {{ teamAFreeThrowPercentage + "%" }}
            </div>
            <div class="font-medium md:text-sm text-xs flex justify-center w-[80px]">
              {{ $t("Free Throw") }}
            </div>
            <div class="font-medium md:text-sm text-xs w-1/3 md:pl-6 pl-0">
          ...
          <div class="flex justify-start items-center w-full" v-show="!isCN">
            ...
            <div class="font-medium md:text-sm text-xs flex justify-center w-[100px]">
              {{ $t("Free Throw") }}
            </div>
        ...
        <div class="flex flex-col justify-around items">
          <div class="flex items-center">
            <div class="bottom-1 team relative w-[50px] p-[20px]">
              ...
              <div
                ...
                :style="{ width: teamBThreeGoal + '%' }"
              ></div>
            </div>
            <p class="w-[30px] font-medium md:text-sm text-10px pl-3 pr-1">
              {{ awayThreeGoal }}
            </p>
          </div>
          <div class="flex items-center" v-show="isCN">
          ...
          <div class="flex items-center" v-show="!isCN">
            <div class="bottom-1 team relative w-[50px] p-[20px]">
              ...
              <div
                class="stat-bar-right right-bar-color border_rightTB"
                :style="{ width: teamBTotalTurnOver + '%' }"
              ></div>
            </div>
            <p class="w-[30px] font-medium md:text-sm text-10px pl-3 pr-1">
              {{ awayTurnOvers }}
...
export default {
  ...
  async mounted() {
    ...
    this.getTournamentDetails = await getBasketBallMatchbyId(
      this.tournamentID,
      this.isCN
    );
    ...
    if (this.getTournamentDetails !== null) {
      ...
      this.homeThreeGoal = this.isCN
        ? parseInt(this.getTournamentDetails["hthreeGoal"]) || 0
        : parseInt(this.getTournamentDetails["hthreePointGoals"]) || 0;
      this.awayThreeGoal = this.isCN
        ? parseInt(this.getTournamentDetails["athreeGoal"]) || 0
        : parseInt(this.getTournamentDetails["athreePointGoals"]) || 0;

      // htwoGoal (2分球得分) , atwoGoal (2分球得分)
      this.awayTwoGoal = parseInt(this.getTournamentDetails["atwoGoal"]) || 0;
      this.homeTwoGoal = parseInt(this.getTournamentDetails["htwoGoal"]) || 0;

      //  hfreeThrows (罚球) afreeThrows (罚球)
      this.homeFreeThrow = this.isCN
        ? parseInt(this.getTournamentDetails["hfreeThrow"]) || 0
        : parseInt(this.getTournamentDetails["hfreeThrows"]) || 0;
      this.awayFreeThrow = this.isCN
        ? parseInt(this.getTournamentDetails["afreeThrow"]) || 0
        : parseInt(this.getTournamentDetails["afreeThrows"]) || 0;

      //  hfreeThrow (罚球率) afreeThrow (罚球率)
      this.homeFreeThrowPercentage = this.isCN
        ? parseInt(this.getTournamentDetails["hfreeThrowPercentage"]) || 0
        : parseInt(this.getTournamentDetails["hfreeThrows"]) || 0;
      this.awayFreeThrowPercentage = this.isCN
        ? parseInt(this.getTournamentDetails["afreeThrowPercentage"]) || 0
        : parseInt(this.getTournamentDetails["afreeThrows"]) || 0;

      //htotalPause (暂停) atotalPause (暂停)
      this.homeTotalPause = parseInt(this.getTournamentDetails["htotalPause"]) || 0;
      this.awayTotalPause = parseInt(this.getTournamentDetails["atotalPause"]) || 0;

      //hnumOfFouls (犯规) anumOfFouls (犯规)
      this.homeNumOfFouls = this.isCN
        ? parseInt(this.getTournamentDetails["hnumOfFouls"]) || 0
        : parseInt(this.getTournamentDetails["hpersonalFouls"]) || 0;
      this.awayNumOfFouls = this.isCN
        ? parseInt(this.getTournamentDetails["anumOfFouls"]) || 0
        : parseInt(this.getTournamentDetails["apersonalFouls"]) || 0;

      //EN Need Data

      //hblocks (Block) ablocks (Block)
      this.homeBlocks = parseInt(this.getTournamentDetails["hblocks"]) || 0;
      this.awayBlocks = parseInt(this.getTournamentDetails["ablocks"]) || 0;

      // hsteals (Steal) asteals (Steal)
      this.homeSteals = parseInt(this.getTournamentDetails["hsteals"] || 0);
      this.awaySteals = parseInt(this.getTournamentDetails["asteals"] || 0);

      // hturnOvers (Turn Over) aturnOvers (Turn Over)
      this.homeTurnOvers = parseInt(this.getTournamentDetails["hturnOvers"]) || 0;
      this.awayTurnOvers = parseInt(this.getTournamentDetails["aturnOvers"]) || 0;

      // hfieldGoals (Field Goal) afieldGoals (Field Goal)
      this.homeFieldGoal = parseInt(this.getTournamentDetails["hfieldGoals"]) || 0;
      this.awayFieldGoal = parseInt(this.getTournamentDetails["afieldGoals"]) || 0;

      //hrebounds (Rebound) arebounds (Rebound)
      this.homeRebounds = parseInt(this.getTournamentDetails["hrebounds"]) || 0;
      this.awayRebound = parseInt(this.getTournamentDetails["arebounds"]) || 0;
    }
  },
